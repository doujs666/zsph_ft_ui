...
class TestTotalFlow(BaseSeleniumTestCase):
    ...
    def test_loan_status(self):
        ...
        for i in range(2,5):
            # 新建客户
            customer_name = u'核桃' + str(i)
            print customer_name
            card_no = str(340827198311170200 + i)
            print card_no
            mobile = str(13522241100 + i)
            print mobile
            print i
            ...
            time.sleep(2)
            status = LoanList(self.selenium).get_loan_status(customer_name)
            ...
            Index(self.selenium).click_user_list().click_user_quit()
            TotalFlow(self.selenium).risk_management_submit_audit(credit_person_login_name, customer_name, self.status)
            Index(self.selenium).click_user_list().click_user_quit()
