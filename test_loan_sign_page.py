...
class TestLoanSignPage(BaseSeleniumTestCase):
    ...
    def test_click_generate_after(self):
        ...
        rel = LoanSignPage(self.selenium).get_contract_status()
    ...
    def test_submit_pass_contract_status(self):
        TestPage(self.selenium).console_login(self.login_name, self.password)
        LoanSignPage(self.selenium).click_apply_button().choose_pass().click_submit_button()
        message = LoanSignPage(self.selenium).get_button_text()
    ...
    def test_submit_contract_status(self):
        TestPage(self.selenium).console_login(self.login_name, self.password)
        LoanSignPage(self.selenium).click_apply_button().choose_reject().click_submit_button()
    ...
    def test_credit_contract_status(self):
        TestPage(self.selenium).console_login(self.login_name, self.password)
        LoanSignPage(self.selenium).click_apply_button().choose_pass().click_submit_button()
        LoanSignPage(self.selenium).click_apply_button().click_submit_button()
